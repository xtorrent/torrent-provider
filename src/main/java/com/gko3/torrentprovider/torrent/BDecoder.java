...
public class BDecoder {
    ...
    private long getNumberFromStreamOld(InputStream bais, char parseChar) throws
        ...
        String strValue = new String(cb.array(), 0, cb.limit());
    ...
    private void print(PrintWriter writer, Object obj, String indent, boolean skipIndent) {
        String useIndent = skipIndent ? "" : indent;

        if (obj instanceof Long) {
            writer.println(useIndent + obj);
        } else if (obj instanceof byte[]) {
            byte[] b = (byte[]) obj;
            if (b.length == 20) {
                writer.println(useIndent + " { " + b + " }");
            } else if (b.length < 64) {
